...
function toggleOptions(deciderNumber) {
    var deciderSelect = document.getElementById("desider" + deciderNumber);
    ...
    for (var i = 1; i <= 3; i++) {
        // Change to 3 if you have 3 dropdowns
        var otherSelectBox = document.getElementById("desider" + i);
...
function updateDivisionOptions() {
    ...
    if (highSchoolValue === "1") {
        // If High School is الثانوية العامة
    ...
    } else if (highSchoolValue === "2") {
        // If High School is الثانوية الازهرية
        divisionSelect.querySelectorAll("option").forEach(function (option) {
            var value = option.getAttribute("value");
            if (value === "3" || value === "4") {
                option.style.display = "block";
...
document
    .getElementsByName("high_school")[0]
    .addEventListener("change", updateDivisionOptions);
